...
const UploadScreen = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (file) {
      ...
      const res = await axios.post(
        'https://youtube-clone-backend-production.up.railway.app/upload',
        fd
      );
  ...
  const fetchFiles = useCallback(async () => {
    const res = await axios.get(
      'https://youtube-clone-backend-production.up.railway.app/files'
    );
    setFiles(res.data);
  }, []);
  ...
  const removeFile = useCallback(
    async (filename, index) => {
      const res = await axios.delete(
        `https://youtube-clone-backend-production.up.railway.app/delete/${filename}`
      );
      if (res.status === 200) {
        let temp = [...files];
        console.log(temp);
        ...
        <Col>
          <form onSubmit={handleSubmit} encType="multipart/form-data">
            <input
              type="file"
              name="file"
              className="p-2 rounded btn btn-gray"
              onChange={handleFile}
              required
            />
          ...
          {files.map((file, i) => (
            <div key={file._id} className="Item">
              <video width={500} height={320} controls>
                <source
                  src={`https://youtube-clone-backend-production.up.railway.app/read/${file.filename}`}
                  type="video/mp4"
                />
              </video>
